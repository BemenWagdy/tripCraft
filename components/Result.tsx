...
const StructuredContent: React.FC<StructuredContentProps> = ({ data, destination }) => {
  if (!data) return null;
  ...
  return (
    <div className="space-y-8">
            ...
            <div className="divide-y divide-gray-200">
              {data.averages.hostel && (
                <div className="p-3 flex justify-between">
                  <span>Hostel</span>
                  <span className="font-bold">${data.averages.hostel}</span>
                </div>
              )}
              {data.averages.midHotel && (
                <div className="p-3 flex justify-between">
                  <span>Mid-range hotel</span>
                  <span className="font-bold">${data.averages.midHotel}</span>
                </div>
              )}
              {data.averages.highEnd && (
                <div className="p-3 flex justify-between">
                  <span>High-end hotel</span>
                  <span className="font-bold">${data.averages.highEnd}</span>
                </div>
              )}
            </div>
          </div>
      ...
      {data.foodList && data.foodList.length > 0 && (
        <div>
          <h2 className="text-xl font-semibold text-gray-800 mb-4 border-b border-gray-200 pb-2">
            Must-Try Food ({data.foodList.length} items)
          </h2>
          <div className="grid gap-3">
            {data.foodList.map((food: any, index: number) => (
              <div key={index} className="bg-gray-50 p-3 rounded-lg">
                <div className="flex justify-between items-start mb-1">
                  <h3 className="font-semibold text-gray-800">{index + 1}. {food.name}</h3>
                  <div className="text-sm text-gray-600">
                    ⭐ {food.rating} ({food.source})
                  </div>
                </div>
                {food.price && (
                  <div className="font-bold text-green-600 text-sm mb-1">{food.price}</div>
                )}
                {food.note && (
